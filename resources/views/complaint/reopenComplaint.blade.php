...
@section('pageTitle', 'Re-Open Complaint')
@section('page-css')
    ...
    <div class="container" style="padding-left: 100px;">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h5 class="panel-title text-muted">Re-Open Complaint</h5>
            </div>
            <div class="panel-body">
                <div class="container">
                    <br>
                    {{Form::open(array('id' => 'assignee','action' => 'ComplaintHandlingController@storeReopenComplaint','method' => 'post', 'role' => 'form', 'invalidate' => 'invalidate', 'files'=>true))}}
                    ...
                    <div class="row{{ $errors->has('ticketnumber') ? ' has-error' : '' }} mt-1">
                        ...
                        <div class="col-sm-6">
                            {{ Form::text('ticketnumber', $ticketno, array('class' => 'form-control','readonly' => true,'style'=>'background-color:white;')) }}
                        </div>
                    </div>
                    <div class="row{{ $errors->has('assignees') ? ' has-error' : '' }} mt-1">
                        ...
                        <div class="col-sm-6">
                            @if($assigneemodel != '')
                            {{ Form::select('assignees',$assigneelist,$assigneename,array('placeholder'=>'--SELECT--','id' => 'assigneesid')) }}
                            @if ($errors->has('assignees'))
                                <span class="help-block"><strong>{{ $errors->first('assignees') }}</strong></span>
                            @endif
                            @else
                                    {{ Form::select('assignees',$assigneelist,null,array('placeholder'=>'--SELECT--','id' => 'assigneesid')) }}
                                    @if ($errors->has('assignees'))
                                        <span class="help-block"><strong>{{ $errors->first('assignees') }}</strong></span>
                                    @endif
                            @endif
                    ...
                    <div class="row{{ $errors->has('reopendescription') ? ' has-error' : '' }}">
                        <label for="input" class="col-sm-4 col-form-label text-muted">Reopening Reason<br/>(Max 500
                            Chars)</label>
                        <div class="col-sm-6">
                            {{ Form::textarea('reopendescription',null,['class'=>'form-control', 'rows' => 3, 'cols' => 40, 'required' => 'required','onKeyPress' => "if(this.value.length==500) return false;"]) }}
                            @if ($errors->has('reopendescription'))
                                <span class="help-block"><strong>{{ $errors->first('reopendescription') }}</strong></span>
                            @endif
                        </div>
                    </div>
                    <br>
...
{{--    </script>--}}
{{--    <script type="text/javascript">--}}
{{--        function getassigneeassigneddata() {--}}
