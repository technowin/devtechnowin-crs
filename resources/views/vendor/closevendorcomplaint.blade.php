...
@section('content')
    ...
    {{Form::open(array('action' => array('VendorController@submitVendorComplaint',$complaints->ticketno),'method' => 'post', 'role' => 'form', 'invalidate' => 'invalidate','files' => true,'id'=>'formid'))}}
        ...
        <div class="form-group" id="descriptiondiv">
            ...
            {{ Form::text('description', $complaints->complaintdescription, array('class' => 'form-control','readonly'=>true)) }}
        </div>
        <div class ="form-group">
            <label class="col-sm-3 col-form-label text-muted">OEM's Ticket No</label>
            {{Form::text('vendorTicketNo',$assignee->calldetails,array('class' => 'form-control','readonly'=>true))}}
        </div>
        <div class="form-group">
        ...
        <div class="form-group" id="complaintstatusiddiv">
            ...
            {{ Form::select('complaintstatus', $statusList, $assignee->assigneestatus, array('class'=>'form-control complaintstatusclassid','placeholder' => '--SELECT--', 'id' => 'complaintstatus')) }}
        </div>
        <div class="form-group" id="resolvecommentdiv">
            <label class="col-sm-3 col-form-label text-muted">Comment</label>
            {{ Form::textarea('resolvecomment', $assignee->ticketresolvecomment, array('id' => 'resolvecomment','class' => 'form-control-sm','rows'=>'3')) }}
        </div>
        <div class="form-group" id="unresolvecommentdiv">
            <label class="col-sm-3 col-form-label text-muted">Un Resolve Comment</label>
            {{ Form::textarea('unresolvecomment', $assignee->ticketunresolvecomment, array('id' => 'unresolvecomment','class' => 'form-control-sm','rows'=>'3')) }}
        </div>
        <div class="form-group" id="pendingreasondiv">
            <label class="col-sm-3 col-form-label text-muted">Pending Reason</label>
            {{ Form::textarea('pendingreason', $assignee->ticketpendingreason, array('id' => 'pendingreason','class' => 'form-control-sm','rows'=>'3')) }}
        </div>
        <div class="form-group" id="nextactionremarkdiv">
            <label class="col-sm-3 col-form-label text-muted">Next Action Remark</label>
            {{ Form::textarea('nextactionremark', $assignee->ticketnextactionremark, array('id' => 'nextactionremark','class' => 'form-control-sm','rows'=>'3')) }}
        </div>
        <div class="form-group" id="nextactionremarkdiv">
            <label class="col-sm-3 col-form-label text-muted">Upload Files</label>
            {{ Form::file('file[]',array('class'=>'form-control form-control-sm uplaodfileclass','multiple'=>true,'multiple id'=>'uplodedfileid','onsubmit'=>'ValidateFileUpload()')) }}
        </div>
    ...
    <script type="text/javascript">
        ...
        function showhideDivs() {
            ...
            if(complaintstatusValueselectedText == "RESOLVED"){
                ...
                document.getElementById("unresolvecomment").required = false;
                document.getElementById("pendingreason").required = false;
                document.getElementById("nextactionremark").required = false;
