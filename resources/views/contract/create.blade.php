                                ...
                                <div class="col card-body">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="row {{ $errors->has('contractno') ? ' has-error' : '' }} mt-2">
                                            <label for="input" class="col-sm-3 col-form-label text-muted">Contract No 676776</label>
                                            <div class="col-sm-6">
                                        ...
                                        <div class="row{{ $errors->has('customers') ? ' has-error' : '' }}">
                                            <label for="input" class="col-sm-3 col-form-label text-muted">Customer Namerrrrrrrrrr</label>
                                            <div class="col-sm-6">
                                        ...
                                        <div class="row{{ $errors->has('totalcost') ? ' has-error' : '' }}">
                                            <label for="input" class="col-sm-3 col-form-label text-muted">Total Cost5555</label>
                                            <div class="col-sm-6">
...
<div class="row">
    <div class="col-md-12">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th width="33%">Document 1</th>
                    <th width="33%">Document 2</th>
                    <th width="34%">Document 3</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>
                        <div id="doc1Container">
                            <button type="button" class="btn btn-primary btn-sm" onclick="uploadDocument('doc1')">
                                <i class="fa fa-upload"></i> Choose File
                            </button>
                            <div id="doc1Info" class="mt-2"></div>
                        </div>
                    </td>
                    <td>
                        <div id="doc2Container">
                            <button type="button" class="btn btn-primary btn-sm" onclick="uploadDocument('doc2')">
                                <i class="fa fa-upload"></i> Choose File
                            </button>
                            <div id="doc2Info" class="mt-2"></div>
                        </div>
                    </td>
                    <td>
                        <div id="doc3Container">
                            <button type="button" class="btn btn-primary btn-sm" onclick="uploadDocument('doc3')">
                                <i class="fa fa-upload"></i> Choose File
                            </button>
                            <div id="doc3Info" class="mt-2"></div>
                        </div>
                    </td>
                </tr>
            </tbody>
...
<script>
// Global variable to track which document is being uploaded
...
function uploadDocument(fieldName) {
    ...
    if (!contractNo || contractNo == '') {
        alert('Please fill in Work Order Type and Work Order No first to generate Contract Number!');
        return;
    }
...
$('#fileInput').on('change', function() {
    ...
    if (file) {
        ...
        // Upload the file
        uploadFileToServer(file, currentUploadField);
    }
});

// Function to upload file to server
function uploadFileToServer(file, fieldName) {
    var contractNo = $('#contractno').val();
    ...
    var loadingHtml = '<div class="alert alert-info alert-sm p-1">Uploading... <i class="fa fa-spinner fa-spin"></i></div>';
    $('#' + fieldName + 'Info').html(loadingHtml);
    $('#' + fieldName + 'Container button').prop('disabled', true);
    
    $.ajax({
        type: "POST",
        url: "{{URL::to('appadmin/upload-contract-document')}}",
        data: formData,
        processData: false,
        contentType: false,
        success: function(response) {
            if (response.success) {
                // Update the info with file details
                var fileSize = (file.size / 1024).toFixed(2);
                var fileType = file.type.split('/')[1].toUpperCase();
                
                var newHtml = '<div class="alert alert-success alert-sm p-1 mb-1">' +
                              '<strong>Uploaded:</strong> ' + file.name + '<br>' +
                              '<small>Size: ' + fileSize + ' KB | Type: ' + fileType + '</small><br>' +
                              '<a href="' + response.file_path + '" target="_blank" class="btn btn-xs btn-info mt-1">View Document</a> ' +
                              '<button type="button" class="btn btn-xs btn-danger mt-1" onclick="deleteDocument(\'' + fieldName + '\')">Delete</button>' +
                              '</div>';
                
                $('#' + fieldName + 'Info').html(newHtml);
                alert('Document uploaded successfully!');
            } else {
                $('#' + fieldName + 'Info').html('<div class="alert alert-danger alert-sm p-1">Upload failed: ' + response.message + '</div>');
            }
            $('#' + fieldName + 'Container button').prop('disabled', false);
        },
        error: function(xhr, status, error) {
            $('#' + fieldName + 'Info').html('<div class="alert alert-danger alert-sm p-1">Error: ' + error + '</div>');
            $('#' + fieldName + 'Container button').prop('disabled', false);
        }
    });
...
function deleteDocument(fieldName) {
    ...
    $.ajax({
        type: "POST",
        url: "{{URL::to('appadmin/delete-contract-document')}}",
        data: {
...
// Load existing documents when contract number is populated
function loadExistingDocuments() {
    var contractNo = $('#contractno').val();
    if (contractNo && contractNo != '') {
        $.ajax({
            type: "GET",
            url: "{{URL::to('appadmin/get-contract-documents')}}/" + contractNo,
            success: function(response) {
                if (response.success && response.documents) {
                    // Load Doc1
                    if (response.documents.doc1) {
                        var doc1Html = '<div class="alert alert-success alert-sm p-1 mb-1">' +
                                      '<strong>Uploaded Document</strong><br>' +
                                      '<a href="' + response.documents.doc1 + '" target="_blank" class="btn btn-xs btn-info mt-1">View Document</a> ' +
                                      '<button type="button" class="btn btn-xs btn-danger mt-1" onclick="deleteDocument(\'doc1\')">Delete</button>' +
                                      '</div>';
                        $('#doc1Info').html(doc1Html);
                    }
                    
                    // Load Doc2
                    if (response.documents.doc2) {
                        var doc2Html = '<div class="alert alert-success alert-sm p-1 mb-1">' +
                                      '<strong>Uploaded Document</strong><br>' +
                                      '<a href="' + response.documents.doc2 + '" target="_blank" class="btn btn-xs btn-info mt-1">View Document</a> ' +
                                      '<button type="button" class="btn btn-xs btn-danger mt-1" onclick="deleteDocument(\'doc2\')">Delete</button>' +
                                      '</div>';
                        $('#doc2Info').html(doc2Html);
                    }
                    
                    // Load Doc3
                    if (response.documents.doc3) {
                        var doc3Html = '<div class="alert alert-success alert-sm p-1 mb-1">' +
                                      '<strong>Uploaded Document</strong><br>' +
                                      '<a href="' + response.documents.doc3 + '" target="_blank" class="btn btn-xs btn-info mt-1">View Document</a> ' +
                                      '<button type="button" class="btn btn-xs btn-danger mt-1" onclick="deleteDocument(\'doc3\')">Delete</button>' +
                                      '</div>';
                        $('#doc3Info').html(doc3Html);
                    }
                }
            }
        });
...
// Call loadExistingDocuments when contract number is set
$(document).ready(function() {
    // Watch for contract number changes
    ...
    var contractNoInput = document.getElementById('contractno');
    if (contractNoInput) {
        observer.observe(contractNoInput, { attributes: true });
        loadExistingDocuments();
    }
});
</script>
